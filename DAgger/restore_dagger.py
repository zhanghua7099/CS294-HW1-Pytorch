...
def load_policy_Dagger(observation):
    dagger_policy_path = './model/Dagger.pth'
    observation = torch.from_numpy(observation)
    observation = Variable(observation).cuda().float()
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    policy = Neural_Network(376, 17).to(device)
    policy.load_state_dict(torch.load(dagger_policy_path))
...
with tf.Session():
    ...
    for i in range(20):
        ...
        steps = 0
        while not done:
            action = load_policy_Dagger(obs[None, :])
